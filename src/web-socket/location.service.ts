...
export class LocationService {
  ...
  async findUsersAhead(
    ...
    try {
      console.log("====",driver.location.coordinates[0], driver.location.coordinates[1]);
      ...
      let API_KEY_ROAD = '5b3ce3597851110001cf62484419ae65a9fe45aead63bcf4d68328e1'
     try {
      const response = await fetch(`https://api.openrouteservice.org/v2/directions/driving-car?api_key=${API_KEY_ROAD}&start=${from.long},${from.lat}&end=${to.long},${to.lat}`);
      ...
      const buffer: any = turf.buffer(line, this.COORIDOOR_SECONDARY, { units: 'meters' });
  ...
  async isUSerAhead(user: locationNow, from: locationNow, buffer) {
    try {
      ...
      const distanceInKm = turf.distance(turf.point([from.long, from.lat]), userPoint, { units: 'kilometers' });
      const distanceInMeters = distanceInKm * 1000;
      const bearing = turf.bearing(turf.point([from.long, from.lat]), userPoint);
      ...
      const directionMatch = await this.isUserInSameDirection([from.long, from.lat], [user.long, user.lat]);
      const shouldAlert = isInDistance && isBearing && isInsideCorridor;
      return { coordinates: user, distanceInMeters, bearing, isInsideCorridor, directionMatch, shouldAlert };
